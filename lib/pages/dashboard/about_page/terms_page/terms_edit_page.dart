...
// UPDATED: Added proper field validation - Publish disabled until ALL fields valid AND changes made
...
class _TermsEditPageState extends State<TermsEditPage> {
  ...
  // Track validation state of each field
  bool _navTitleEnValid = false;
  bool _navTitleArValid = false;
  bool _termsDescEnValid = false;
  bool _termsDescArValid = false;
  bool _privacyDescEnValid = false;
  bool _privacyDescArValid = false;
  ...
  /// Computed live — true when every required field is valid
  bool get _isFormValid {
    return _navTitleEnValid &&
        _navTitleArValid &&
        _navIcon.hasImage &&
        _termsDescEnValid &&
        _termsDescArValid &&
        ...
        _privacyDescEnValid &&
        _privacyDescArValid &&
        _privacySvg.hasImage;
  }

  // Validation methods
  bool _validateNavTitleEn(String value) {
    final trimmed = value.trim();
    return trimmed.isNotEmpty && trimmed.length <= 200;
  }

  bool _validateNavTitleAr(String value) {
    final trimmed = value.trim();
    return trimmed.isNotEmpty && trimmed.length <= 200;
  }

  bool _validateDescriptionEn(String value) {
    final trimmed = value.trim();
    return trimmed.isNotEmpty && trimmed.length <= 800;
  }

  bool _validateDescriptionAr(String value) {
    final trimmed = value.trim();
    return trimmed.isNotEmpty && trimmed.length <= 800;
  }
  ...
  void initState() {
    ...
    // Add listeners to detect changes and validate
    _navTitleEnCtrl.addListener(() {
      setState(() {
        _navTitleEnValid = _validateNavTitleEn(_navTitleEnCtrl.text);
      });
      _checkForChanges();
    });

    _navTitleArCtrl.addListener(() {
      setState(() {
        _navTitleArValid = _validateNavTitleAr(_navTitleArCtrl.text);
      });
      _checkForChanges();
    });

    _termsDescEnCtrl.addListener(() {
      setState(() {
        _termsDescEnValid = _validateDescriptionEn(_termsDescEnCtrl.text);
      });
      _checkForChanges();
    });

    _termsDescArCtrl.addListener(() {
      setState(() {
        _termsDescArValid = _validateDescriptionAr(_termsDescArCtrl.text);
      });
      _checkForChanges();
    });

    _privacyDescEnCtrl.addListener(() {
      setState(() {
        _privacyDescEnValid = _validateDescriptionEn(_privacyDescEnCtrl.text);
      });
      _checkForChanges();
    });

    _privacyDescArCtrl.addListener(() {
      setState(() {
        _privacyDescArValid = _validateDescriptionAr(_privacyDescArCtrl.text);
      });
      _checkForChanges();
    });
  }
  ...
  void _checkForChanges() {
    ...
    final bool hasDocChanges =
        _termsDocEn.bytes != null ||
            ...
            (_termsDocEn.existingUrl != _originalTermsDocEnUrl && _termsDocEn.bytes == null) ||
            (_termsDocAr.existingUrl != _originalTermsDocArUrl && _termsDocAr.bytes == null) ||
            (_privacyDocEn.existingUrl != _originalPrivacyDocEnUrl && _privacyDocEn.bytes == null) ||
            (_privacyDocAr.existingUrl != _originalPrivacyDocArUrl && _privacyDocAr.bytes == null);
  ...
  // ── File pickers (SVG only for images) ────────────────────────────────────
  Future<Uint8List?> _pickSvgOnly() async {
    ...
    input.onChange.listen((_) {
      ...
      if (!file.name.toLowerCase().endsWith('.svg')) {
        _showErrorDialog('Invalid File Type', 'Please select an SVG file only. Selected: ${file.name}');
        c.complete(null);
        return;
      }

      if (file.type != 'image/svg+xml') {
        _showErrorDialog('Invalid File Type', 'Please select a valid SVG file. Selected: ${file.name}');
        c.complete(null);
        return;
      }
      ...
      reader.onLoadEnd.listen((_) {
        ...
        if (bytes != null) {
          final headerStr = String.fromCharCodes(bytes.sublist(0, bytes.length > 100 ? 100 : bytes.length));
          if (!headerStr.contains('<svg') && !headerStr.contains('<?xml')) {
            _showErrorDialog('Invalid SVG', 'The selected file does not appear to be a valid SVG image.');
            c.complete(null);
            return;
          }
          c.complete(bytes);
  ...
  // ── Seed — called on every TermsLoaded ────────────────────────────────────
  ...
  List<String> _getMissingFields() {
    ...
    if (!_navIcon.hasImage) {
      missing.add('Navigation Icon (SVG)');
    }
    if (!_termsDescEnValid) {
      missing.add('Terms & Conditions Description (English)');
    }
    if (!_termsDescArValid) {
      missing.add('Terms & Conditions Description (Arabic)');
    }
    if (!_termsSvg.hasImage) {
      missing.add('Terms & Conditions SVG Icon');
    }
    if (!_privacyDescEnValid) {
      missing.add('Privacy Policy Description (English)');
    }
    if (!_privacyDescArValid) {
      missing.add('Privacy Policy Description (Arabic)');
    }
    if (!_privacySvg.hasImage) {
      missing.add('Privacy Policy SVG Icon');
    }
        ...
        if (state is TermsSaved) {
          ...
          if (mounted) {
            showSuccessDialog(
              context: context,
              title: 'Published',
              subtitle: 'Terms of Service page published successfully!',
            );
      ...
      children: [
        ...
        _accordion(
          ...
          child: Column(
            ...
            children: [
              ...
              if (_submitted && !_navIcon.hasImage)
                Padding(
        ...
        _accordion(
          ...
          child: _sectionEditor(
            ...
            showSvgError: _submitted && !_termsSvg.hasImage,
          ),
        ),
        SizedBox(height: 16.h),
        ...
        _accordion(
          ...
          child: _sectionEditor(
            ...
            showSvgError: _submitted && !_privacySvg.hasImage,
          ),
        ),
